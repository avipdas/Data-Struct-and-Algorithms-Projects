...
void WriteVector(ostream& output, const vector<string>& vec,
   string delimiter = ",");
...
bool VerifyCallbacks(ostream& testFeedback, const vector<string>& actual,
   ...
   WriteVector(testFeedback, expected);
   testFeedback << " }" << endl;
   testFeedback << "  Actual:   { ";
   WriteVector(testFeedback, actual);
   testFeedback << " }" << endl;

   return areEqual;
}

// Writes comma-separated vector elements to the output stream
void WriteVector(std::ostream& output, const std::vector<string>& vec,
